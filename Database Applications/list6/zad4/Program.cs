...
namespace LibraryMongo
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Id = ObjectId.GenerateNewId(),
                Isbn = "979-83-01-00002-8",
                Title = "W Pustyni i w Puszczy",
                Authors = new List<string> { "Henryk Sienkiewicz" },
                Year = 1911,
                Copies = new List<BookCopy>
                {
                    new BookCopy { CopyNumber = 1, Shelf = "B2", Condition = "good" },
                    new BookCopy { CopyNumber = 2, Shelf = "A1", Condition = "good" }
                }
            };

            booksCollection.InsertMany(new[] { book1, book2, book3 });
            ...
            var pagedBooks = booksCollection
                ...
                .Sort(Builders<Book>.Sort.Ascending(b => b.Title))
                .Skip(pageIndex * pageSize)
                .Limit(pageSize)
                .ToList();

            // 2. 
            var filter = Builders<Book>.Filter.ElemMatch(b => b.Copies,
                copy => copy.Shelf == "A1" && copy.Condition == "good");    
            var matchingBooks = booksCollection.Find(filter).ToList();

            Console.WriteLine("Paged Books:");
            pagedBooks.ForEach(b => Console.WriteLine($"{b.Title}"));

            Console.WriteLine("\nBooks with good copy on shelf A1:");
            matchingBooks.ForEach(b => Console.WriteLine($"{b.Title}"));
        }
    }
}
